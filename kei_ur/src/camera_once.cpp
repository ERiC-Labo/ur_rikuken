...
void get_one_message(std::string topic_name, ros::NodeHandle nh, int num, Ty &sensor_msgs)
{
    boost::shared_ptr<const Ty> ptr;
    ptr = ros::topic::waitForMessage<Ty>(topic_name, nh, ros::Duration(num));
    if (ptr != NULL)
    {
        sensor_msgs = *ptr;
    } 
...
int main(int argc, char** argv)
{
    ...
    while (count++ <= 10)
    {
        ...
        ROS_INFO_STREAM("width: " << cam_msgs.width << "  heighr: " << cam_msgs.height);
